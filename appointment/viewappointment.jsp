                    ...
                    <div class="container">
                        ...
                        <ul class="nav navbar-nav navbar-left">
                            ...
                            <li class="hidden-xs hidden-sm"	>
                                <font class="page-title">VIEW HEALTH APPOINTMENT</font>
                            </li>
                        </ul>
			...
			<div class="content">
                    ...
                    <div class="container">
                     ...
                     <div class="panel-heading">
                                 	...
                                 	<div class="row">
                                  <form:form action="viewappointments"  modelAttribute="healthAppointmentDTO" method="post" class="form-inline"> 			
                        		<div class="panel-heading">
                                 			<div class="form-group">
                                 		
                                 		<div class="form-group">
	 <label class="" for="email">From:</label>
<form:input type="text" path="appointmentDate" value="${fromdate}" id="datepicker-autoclose" class="form-control input-sm"  />
</div>

<div class="form-group">
 <label class="" for="email">To:</label>
<form:input type="text" path="appointmentTime" value="${toDate}" id="datepicker-autoclose1" class="form-control input-sm"  />
</div>

<div class="form-group">
	 <label class="" for="email">Service:</label>
	
<form:select path="serviceName" title="select service name"  id="assign" class="form-control input-sm">
																<option value="${healthAppointmentDTO.serviceName}" >${healthAppointmentDTO.serviceName}</option>
															<c:forEach items="${allservice}" var="serv">
																<option  value="${serv.serviceName}">${serv.serviceName}</option>
																</c:forEach>
																</form:select>
</div>

<div class="form-group">
 <label class="" for="email">App.Status:</label>
<form:select path="statusAppointment" title="Appointment Status"  id="assign" class="form-control input-sm">
																<form:option value="" label="select Application status" ></form:option>
																<form:option path="statusAppointment" value="New">New</form:option>
																<form:option path="statusAppointment" value="Availed">Availed</form:option>
																<form:option path="statusAppointment" value="Cancelled">Cancelled</form:option>
																<form:option path="statusAppointment" value="Rescheduled">Rescheduled</form:option>
																<form:option path="statusAppointment" value="Voucher Received">Voucher Received</form:option>
																<form:option path="statusAppointment" value="Missed">Missed</form:option>
																<form:option path="statusAppointment" value="Feed Back">Feed Back</form:option>
																</form:select>
</div>	
<div class="form-group">
<button type="submit" class="btn btn-default btn-rounded btn-sm waves-effect waves-light" value="Go"><i class="fa fa-search" aria-hidden="true"></i> Search</button>
 ...
 <table id="example" class="grid display nowrap" style="width: 100%">
                                        ...
                                        <tbody>
                                            <c:forEach items="${appointment}" var="app" varStatus="counter">
                                            
                        <tr>
                        	<td><c:out value="${counter.index + 1}"/></td>
                        	<td><a href="viewAppointmentsById?appointmentId=<c:out value='${app.appointmentId}'/>"><c:out value="${app.customerName}"/></a></td>
                        	<td><c:out value="${app.nameHcc}"/></td>
                        	<td><c:out value="${app.appointmnetFor}"/></td>
                        	<td><c:out value="${app.serviceName}"/></td> 
                        	<td><c:out value="${app.appointmentDate}"/></td> 
                        	<td><c:out value="${app.appointmentTime}"/></td> 
                        	<td><c:out value="${app.source}"/></td> 
                        	<td>
                        	<c:if test="${role == 'Admin' || role == 'Customer Care'}">
                        	<c:if test="${app.statusAppointment == 'New'}">
                        	<p class="btn btn-warning btn-xs"><a href="editAppointment?appointmentId=<c:out value='${app.appointmentId}'/>"><c:out value="${app.statusAppointment}"/></a></p> 
                        	</c:if>
                        	<c:if test="${app.statusAppointment == 'Availed' || app.statusAppointment == 'Confirmed'}">
                        	<p class="btn btn-success btn-xs"><a href="viewAppointmentsById?appointmentId=<c:out value='${app.appointmentId}'/>">Availed</a></p> 
                        	</c:if>
                        	<c:if test="${app.statusAppointment == 'Feed Back'}">
                        	<p class="btn btn-info btn-xs"><a href="customerFeedBack?appointmentId=<c:out value='${app.appointmentId}'/>">Feed Back</a></p> 
                        	</c:if>
                        	<c:if test="${app.statusAppointment == 'Rescheduled' || app.statusAppointment =='Missed'}">
                        	<p class="btn btn-primary btn-xs"><a href="editAppointment?appointmentId=<c:out value='${app.appointmentId}'/>"><c:out value="${app.statusAppointment}"/></a></p> 
                        	</c:if>
                        	<c:if test="${app.statusAppointment == 'Cancelled'}">
                        	<p class="btn btn-danger btn-xs"><c:out value="${app.statusAppointment}"/></p> 
                        	</c:if>
                        	<c:if test="${app.statusAppointment == 'Voucher Received'}">
                        	<p class="btn btn-info btn-xs"><a href="viewAppointmentsById?appointmentId=<c:out value='${app.appointmentId}'/>"><c:out value="${app.statusAppointment}"/></a></p> 
                        	</c:if>
                        	</c:if>
                        	</td>
                        	<%-- <td>
                        	<a href="viewAppointmentsById?appointmentId=<c:out value='${app.appointmentId}'/>"><i class="fa fa-eye" aria-hidden="true"></i></a>
                        	<a href="editAppointment?appointmentId=<c:out value='${app.appointmentId}'/>"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a></td> --%>
                        </tr>
                    </c:forEach>
                                        </tbody>
                                    </table>
