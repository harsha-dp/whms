...
<body class="fixed-left">
				...
				<div class="container">
					...
					<ul class="nav navbar-nav navbar-left">
					 <li>
						...
						<li class="hidden-xs hidden-sm"	><font face="verdana"  class="page-title">APPOINTMENT AVAILED</font> <!-- <h4 class="page-title">Edit Presales</h4> --></li>
					</ul>
 ...
 <table id="example" class="grid display nowrap" style="width: 100%">
                                       ...
                                       <tbody>
                          <c:forEach items="${allVoucherEntry}" var="appointment" varStatus="counter">
                            <tr>
                                         <form:form action="saveVouchers" modelAttribute="healthApptTO">
                                            <form:hidden path="appointmentId" value="${appointment.appointmentId}" />
                        <td><c:out value="${counter.index + 1}" /></td>
                        	<td class="account-name sort-value"><a href="viewcustomersById?customerId=<c:out value='${appointment.customerId}'/>"><c:out value="${appointment.CustomerName}" /></a></td>
                        	<td class="account-name1 sort-value"><c:out value="${appointment.UsedBy}" /></td>
                       		<td class="account-name2 sort-value"><c:out value="${appointment.Category}"/></td>
                       		<td class="account-name3 sort-value"><c:out value="${appointment.Service}" /></td>
                        	<td class="account-name4 sort-value"><c:out value="${appointment.ServiceCentre}" /></td>
                       		<td class="account-name5 sort-value"><c:out value="${appointment.ApptDate}"/></td>
                       		<td class="account-name6 sort-value">
                       		<c:set var="voucher" value="${appointment.voucherNo}"/>
						<c:set var="voucherNo" value="${fn:split(voucher, ' ')}" />
                       		 <select  array="voucherNo" name="txOffersVoucher">
        	<c:forEach var="numbers" items="${voucherNo}">
            <option name="txOffersVoucher" value="${numbers}"><c:out value="${numbers}"/>
            </option>
        </c:forEach>
    </select>
                       		</td>
                        	<td>
                        	<button class="btn btn-primary btn-xs"  data-target="#edit"  onclick="return confirm('Are you sure!!Do you want to submit?' )" >Submit</button></td>
