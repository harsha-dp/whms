                        ...
                        <ul class="nav navbar-nav navbar-left">
                         <li>
                            ...
                            <li class="hidden-xs hidden-sm">
                                <font class="page-title">VIEW INCOMING CALLS DETAILS</font>
                            </li>
                        </ul>
			...
			<div class="content">
                    <div class="container">
                     ...
                     <div class="panel-heading">
                                 	<div class="row">
                                  <form action="incomingCalls1"  method="post"> 			
                        		<div class="panel-heading">
                                 			<div class="form-group">
			                                			<div class="col-sm-10">
			                                				<div class="input-daterange input-group">
																...
																<span class="input-group-addon bg-default b-0 text-black">Calls Mode</span>
																<select name="callsMode" title="s"  id="assign" class="form-control input-sm">
																<option value="${callsMode}" >${callsMode}</option>
	                    ...
	                    <div class="row">
 						...
 						<table id="example" class="grid display nowrap" style="width: 100%">
                                        ...
                                        <tbody>
                                            <c:forEach items="${incomingCallsList}" var="calls" varStatus="counter">
                        <tr>
                         <form action="saveIncomingCallsStatus" > 
                                <input type="hidden" name="id" value="${calls.id}" />
                                <input type="hidden" name="fromDate" value="${fromDate}" />
                                <input type="hidden" name="toDate"  value="${toDate}" />
                                <input type="hidden" name="updatedBy" value="<%=id%>"	/>
                                 <input type="hidden" name="callsMode"  value="${callsMode}" />
                        	<td><c:out value="${counter.index + 1}"/></td>
                        	<td><c:out value="${calls.phoneNo}"/></td>
                        	<td><c:out value="${calls.callDate}"/></td>
                        	<td> <c:if test="${calls.status ne null}"><c:out value="${calls.status}"></c:out></c:if>
                        	    <c:if test="${calls.status == null}">
                              <select  name="status">
                            <c:forEach items="${welezoConstant}" var="list">
										<c:if test="${list.constType == 'Incoming Calls Status'}">
                              ...
                              <td><c:if test="${calls.status ne null}"><c:out value="${calls.purpose}"></c:out></c:if>
                        	    <c:if test="${calls.status == null}">
                              <select  name="purpose">
                             <c:forEach items="${welezoConstant}" var="list">
										<c:if test="${list.constType == 'Purpose'}">
										<option value="${list.CValue}">${list.CDisplay}</option>
										</c:if>
							</c:forEach>
                              </select>
                                 ...
                                 <td>
                                 <c:if test="${calls.status ne null}"><c:out value="${calls.remarks}"></c:out></c:if>
                              <c:if test="${calls.status == null}">
                                 <textarea name="remarks" ></textarea> </c:if>
                                 </td>
                           <td> <c:if test="${calls.status == null}">  
                           <button class="btn btn-primary btn-xs"  data-target="#edit"  >Submit</button></c:if></td>
                             </form> 
                        </tr>
                    </c:forEach>
                                        </tbody>
