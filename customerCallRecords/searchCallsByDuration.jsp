...
.table, td, th {
 ...
 <!--add file validation  -->
<script src="resources/validation/customerCallRecords/searchCallsByDuration.js"></script>
<!--  -->
				...
				<div class="container">
					...
					<ul class="nav navbar-nav navbar-left">
					  <li>
						...
						<li class="hidden-xs hidden-sm"><font class="page-title">SEARCH CALLS DURATION</font></li>
					</ul>
					...
					<div id="myDIV">
						<div class="panel panel-default">
							<div class="panel-heading">
								...
								<div class="row">
									<form action="viewCallsByDurations"  name="form" onsubmit="return validate()" class="form-inline"
										method="post">
										<div class="panel-heading">
	<div class="col-sm-12" style="width:105%;">										
	<div class="form-group">
      <label class="" for="email">From Date:</label>
      <input type="text" id="datepicker-autoclose" name="fromDate" class="form-control input-sm" value="${fromDate}">
    </div>
   
    <div class="form-group">
      <label class="" for="pwd">To:</label>
     <input type="text" id="datepicker-autoclose1" name="toDate" class="form-control input-sm" value="${toDate}">
    </div>
   	
	<div class="form-group">
      <label class="" for="email">ExtNo :</label>
     <select name="extension"  class="form-control input-sm" id="extensionno"  >
                              <option value="${extension}" label="Select Extension Number">${extension}</option>
                                  <c:forEach var="extensionDetails" items="${extensionDetails}">
						<option  Value="${extensionDetails.extensionNo}">${extensionDetails.extensionNo}</option>
					</c:forEach>
			</select>
    </div>
    
    <div class="form-group">
      <label class="" for="pwd">Duration From(In Sec):</label>
    <input type="text" value="0" id="state-success" name="fromDuration" class="form-control input-sm" placeholder="00:00">
    </div>
     <div class="form-group">
      <label class="" for="email">To:</label>
     <input type="text" value="3600"
															id="state-success" name="toDuration"
															class="form-control input-sm" placeholder="23:59">
    </div>
     ...
     <button type="submit" onclick="return validate()"
												...
												<!-- id="date-range" -->
													...
													<div class="col-sm-2">
														...
														<div class="col-sm-2">	
														<input type="text" id="datepicker-autoclose1" name="toDate"
															class="form-control input-sm" value="${toDate}">
															</div>
														<div class="col-sm-2">	
														<span class="input-group-addon bg-default b-0 text-black">ExtNo</span>
															 <select name="extension"  class="form-control input-sm" id="extensionno"  >
                              <option value="${extension}" label="Select Extension Number">${extension}</option>
                                  <c:forEach var="extensionDetails" items="${extensionDetails}">
						<option  Value="${extensionDetails.extensionNo}">${extensionDetails.extensionNo}</option>
					...
					<div class="col-sm-2">
														 <span class="input-group-addon bg-default b-0 text-black">Duration
															From(In Sec)</span> <input type="text" value="0"
															id="state-success" name="fromDuration"
															class="form-control input-sm" placeholder="00:00">
</div>
<div class="col-sm-3">
														<span class="input-group-addon bg-default b-0 text-black">To
															</span> <input type="text" value="3600"
															id="state-success" name="toDuration"
															class="form-control input-sm" placeholder="23:59">
															</div>
												</div> --%>
											 ...
											 <font id="extensionnoerror" style="color:red"></font>
													...
													<tbody>
														<c:forEach var="row" items="${stkList}">
															<tr>
																...
																<c:if test="${columnName.size() >= 1}">
																	<td>${row.attr1}</td>
																</c:if>
																<c:if test="${columnName.size() >= 2}">
																	<td>${row.attr2}</td>
																</c:if>
																<c:if test="${columnName.size() >= 3}">
																	<td>${row.attr3}</td>
																</c:if>
																<c:if test="${columnName.size() >= 4}">
																	<td>${row.attr4}</td>
																</c:if>
																<c:if test="${columnName.size() >= 5}">
																	<td>${row.attr5}</td>
																</c:if>
																<c:if test="${columnName.size() >= 6}">
																	<td>${row.attr6}</td>
																</c:if>
																<c:if test="${columnName.size() >= 7}">
																	<td>${row.attr7}</td>
																</c:if>
																<c:if test="${columnName.size() >= 8}">
																	<td>${row.attr8}</td>
																</c:if>
																<c:if test="${columnName.size() >= 9}">
																	<td>${row.attr9}</td>
																</c:if>
																<c:if test="${columnName.size() >= 10}">
																	<td>${row.attr10}</td>
																</c:if>
																<c:if test="${columnName.size() >= 11}">
																	<td>${row.attr11}</td>
																</c:if>
																<c:if test="${columnName.size() >= 12}">
																	<td>${row.attr12}</td>
																</c:if>
																<c:if test="${columnName.size() >= 13}">
																	<td>${row.attr13}</td>
																</c:if>
																<c:if test="${columnName.size() >= 14}">
																	<td>${row.attr14}</td>
																</c:if>
																<c:if test="${columnName.size() >= 15}">
																	<td>${row.attr15}</td>
																</c:if>
																<c:if test="${columnName.size() >= 16}">
																	<td>${row.attr16}</td>
																</c:if>
																<c:if test="${columnName.size() >= 17}">
																	<td>${row.attr17}</td>
																</c:if>
																<c:if test="${columnName.size() >= 18}">
																	<td>${row.attr18}</td>
																</c:if>
																<c:if test="${columnName.size() >= 19}">
																	<td>${row.attr19}</td>
																</c:if>
																<c:if test="${columnName.size() >= 20}">
																	<td>${row.attr20}</td>
																</c:if>
	...
	<!-- Datatable init js -->
	<scriptsrc"resources/pages/datatables.init.js"></script>
