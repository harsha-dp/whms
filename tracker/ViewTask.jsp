                    ...
                    <div class="container">
						...
						<div class="row">
							<div class="col-sm-12">
								...
								Assignee :
								 <select name="name" title="----vendorame----">
											...
											<c:forEach var="user" varStatus="status" items="${allUser}">
											<c:if test="${user.roleType == 'Admin'}">
                          ...
                          <c:forEach var="row" items="${stkList}"> 
                                <tr>
                                   <c:if test="${columnName.size() >= 1}"><td>
                                   <c:if test="${row.attr8 == userById.userName}">
                                   <a href="editTask?taskId=<c:out value='${row.attr1}'/>&userId=<c:out value="${userById.userId}" />"><i class="fa fa-pencil" aria-hidden="true"></i>Edit</a>
                                   </c:if>
                                   <a href="viewTaskthruId?taskId=<c:out value='${row.attr1}'/>&userId=<c:out value="${userById.userId}" />"><i class="fa fa-eye" aria-hidden="true"></i>View
                                  </a> </td></c:if>
