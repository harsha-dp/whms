(function($$1) {
	...
	var FilterMenu = function() {
		function FilterMenu(target, th, column, index, options) {
			...
			this.column = column;
			this.index = index;
			this.tds = target.find(
					'tbody tr td:nth-child(' + (this.index + 1) + ')')
					.toArray();
		}
		...
		FilterMenu.prototype.searchToggle = function(value) {
			...
			this.inputs.filter(
					function(input) {
						return input.value.toLowerCase().indexOf(
								value.toLowerCase()) > -1;
		...
		FilterMenu.prototype.updateSelectAll = function() {
			if (this.selectAllCheckbox instanceof HTMLInputElement) {
				...
				this.selectAllCheckbox.checked = this.inputs.length === this.inputs
		...
		FilterMenu.prototype.dropdownFilterItemSelectAll = function() {
			...
			dropdownFilterItemSelectAll.innerHTML = dropdownFilterItemSelectAll.innerHTML
					+ ' ' + value;
		...
		FilterMenu.prototype.dropdownFilterContent = function() {
			 ...
			 * this.dropdownFilterSort('A to Z'), this.dropdownFilterSort('Z to
			 * A') ]);
			 */
		...
		FilterMenu.prototype.dropdownFilterDropdown = function() {
			...
			arrow.className = 'glyphicon glyphicon-filter dropdown-filter-icon';
	...
	var FilterCollection = function() {
		...
		FilterCollection.prototype.bindCheckboxes = function() {
			...
			this.target.find('.dropdown-filter-menu-item.item').change(
					function() {
		...
		FilterCollection.prototype.bindSelectAllCheckboxes = function() {
			...
			var ths = this.ths;
			var updateRowVisibility = this.updateRowVisibility;
			this.target.find('.dropdown-filter-menu-item.select-all').change(
					function() {
						var index = $(this).data('index');
		...
		FilterCollection.prototype.updateRowVisibility = function(filterMenus,
				rows, ths) {
			var showRows = rows;
			var hideRows = [];
			...
			for (var i = 0; i < rows.length; i++) {
				...
				for (var j = 0; j < selectedLists.length; j++) {
					var content = tds[selectedLists[j].column].textContent
							.trim().replace(/ +(?= )/g, '');
