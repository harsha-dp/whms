			...
			<div class="navbar navbar-default" role="navigation">
				<div class="container">
					...
					<ul class="nav navbar-nav navbar-left">
						...
						<li class="hidden-xs hidden-sm"><font class="page-title">INVENTORY
								INWARD/OUTWARD REPORT</font></li>
					</ul>
					...
					<div id="myDIV">
						<div class="panel panel-default">
							<div class="panel-heading">
								...
								<div class="row">
									<form action="inventoryReports" class="form-inline">
										
										<div class="panel-heading">
											<div class="form-group">
												<label class="" for="email">From:</label> 
												<input type="text"
													name="fromDate" id="datepicker-autoclose" value="${fromDate}"
													class="form-control input-sm" />
											</div>

											<div class="form-group">
												<label class="" for="email">To:</label> <input type="text" value="${toDate}"
													name="toDate" id="datepicker-autoclose1"
													class="form-control input-sm" />
											</div>
	
											  <div class="form-group">
											  <label class="" for="email">Report Name :</label>
												<select name="entryPath" title="----Select Name----" id="assign" class="form-control input-sm">
																<option  value="" label="select name" ></option>
																<c:forEach var="item" varStatus="status" items="${entryItems}">
										...
										<div class="col-sm-12">
											<div class="scrollit">
												<table id="example" class="grid display nowrap"
													...
													<tbody>
														  <c:forEach items="${stkList}" var="collections" varStatus="counter">
										                      <tr style="display: table-row;">
										                        	<td><c:out value="${counter.index+1}"/></td>
										                        	<td><c:out value="${collections.itemCategory}"/></td>
										                        	 
										                        	 <td> 
										                        	 	
										                        		 <c:choose>
											                        	 	<c:when test="${entryPath=='OutWard'}">
											                        	 		<a href="itemReportOut?itemId=<c:out value='${collections.itemId}'/>&entryType=${entryPath}&dateFrom=${dateFrom}&dateTo=${dateTo}">
											                        	 			 <c:out value="${collections.itemName}"/>
											                        	 		</a>
											                        	 	</c:when>
											                        	 	<c:when test="${entryPath=='Reissue'}">
											                        	 		<a href="itemReportOut?itemId=<c:out value='${collections.itemId}'/>&entryType=${entryPath}&dateFrom=${dateFrom}&dateTo=${dateTo}">
											                        	 			 <c:out value="${collections.itemName}"/>
											                        	 		</a>
											                        	 	</c:when>	
											                        	 	<c:otherwise>										                        	 	
											                        	 		<p> <c:out value='${collections.itemName}'/> </p>
											                        	 	</c:otherwise>
										                        	 	</c:choose>
										                        	 </td>
										                        	...
										                        	<%--   <td> 
										                        	 		...
										                        	 		<a href="itemReportOut?itemId=<c:out value='${collections.itemId}'/>">
										                        	 			 <c:out value="${collections.itemName}" />
										                        	 ...
										                        	 <td>
										                        	 <c:if test="${(collections.currentStock<=2) && (collections.currentStock>=1) }">
																			<p class="label label-warning ">${collections.currentStock}</p>&nbsp ${collections.unitOfMeasure}
																	 </c:if> 
																	 <c:if test="${(collections.currentStock==0)}">
																			<p class="label label-danger">${collections.currentStock} </p>&nbsp ${collections.unitOfMeasure}
										                        	 		<c:out value="${collections.currentStock}"/>
										                        	 </c:if>
										                        	 <c:if test="${(collections.currentStock>=3)}">
										                        	 		<p>${collections.currentStock} </p>&nbsp ${collections.unitOfMeasure}
										                        	 </c:if>		
										                        	 </td>
										                        	 <td><c:out value="${collections.vendorCompany}"/></td>
										                        	  <td><c:out value="${collections.entryType}"/></td>
										                        	    <td><c:out value="${collections.empName}"/></td>
										                        	   <td><c:out value="${collections.actionDate}"/></td>
